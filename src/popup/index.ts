...
class PopupManager {
   ...
   */
  private displayTimeStats(timeData: Record<string, TimeData>): void {
    const siteStatsContainer = document.getElementById('site-stats');
    
    if (!siteStatsContainer) return;
   ...
   */
  private createSiteStatElement(site: string, data: TimeData): HTMLElement {
   ...
   */
  private updateBlockedSitesUI(): void {
   ...
   */
  private createSiteElement(domain: string, config: SiteConfig): HTMLElement {
   ...
   */
  public toggleSite(domain: string): void {
   ...
   */
  public updateSiteTimeLimit(domain: string, value: string): void {
    try {
   ...
   */
  public removeSite(domain: string): void {
  ...
  private setupEventListeners(): void {
    ...
    const customSiteInput = document.getElementById('custom-site') as HTMLInputElement;
    if (customSiteInput) {
    ...
    // Reset today button
    const resetButton = document.getElementById('reset-today') as HTMLButtonElement;
    if (resetButton) {
  ...
  private async addCustomSite(): Promise<void> {
    const customSiteInput = document.getElementById('custom-site') as HTMLInputElement;
    const customTimeLimitInput = document.getElementById('custom-time-limit') as HTMLInputElement;
    const addButton = document.getElementById('add-site') as HTMLButtonElement;

    if (!customSiteInput || !addButton) return;
   ...
   * Save settings
   */
  private async saveSettings(showMessage: boolean = true): Promise<void> {
    try {
      const settings = {
   ...
   * Reset today's data
   */
  private async resetTodayData(): Promise<void> {
   ...
   */
  private showMessage(text: string, type: 'success' | 'error' | 'info' | 'warning' = 'info'): void {
